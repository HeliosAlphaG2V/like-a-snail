...
class emulatorAggregator:
    ...
    def emu(cls):  # loop, root, canvas
        ...
        cls.logFile = sys.argv[1]
        cls.bootRom = sys.argv[2]
        cls.gameRom = sys.argv[3]
        ...
        pygame.init()
        pygame.font.init()
        cls.base_font = pygame.font.Font(None, 32)
    ...
    #    asyncio.set_event_loop(loop)
        ...
        #  Init
        memCntr = MemCntr(cls.bootRom, cls.gameRom)
    ...
    #                 print("Timing error on: " + str(executionTimeReal) + "/" + str(executionTimeShall) )
                ...
                if(memCntr.memory[0xFF40] & (1 << 7) != 0):
                    ...
                    if(scanLineCycles <= 0):
                        ...
                        # Vertical blank period at 144 to 153 (> 153 : Frame is
                        # complete)
                ...
                # Display is off
                    ...
                    scanLineCycles = 216
                    memCntr.memory[0xFF44] = 0
                    memCntr.memory[0xFF41] = (
                        memCntr.memory[0xFF41] & 0xFE) | 0x01
                #########################################################
    ...
    #                         print( format(spriteArray[count, 2], '02X') )
                ...
                cls.drawSprites(sfSprite, spriteArray, tileArray)
                cls.drawTiles(sfTiles, tileArray)
                cls.drawBG(sfScreen, tileArray, memCntr)
                screen.blit(sfTiles, rectTiles)

            if(updateScene):
                cls.updateSceneFC(memCntr, screen, rxOld, ryOld, imgGB, sfScreen, rectScreen,
                                  sfBorder, rectBorder, sfSprite, sfTiles, rectTiles, rectSfSprite, opCode, font)
                updateScene = False

    #         if( time.time() - start_time != 0):
    # print("Time: ", (time.time() - start_time)) # FPS = 1 / time to process
    # loop
    ...
    @classmethod
    def updateSceneFC(cls, memCntr, screen, rxOld, ryOld, imgGB, sfScreen, rectScreen, sfBorder, rectBorder, sfSprite, sfTiles, rectTiles, rectSfSprite, opCode, font):
        ...
        strArray = [str("IME: ") + str(format(memCntr.getMemValue(0xFFFF), "08b")),
                    ...
                    str("OC: ") + str(format(opCode, "04X") +
                                      str(" PC: ") + format(memCntr.getPC(), "04X")),
        ...
        text_surface = cls.base_font.render(
            cls.user_text, True, (255, 255, 255))
        # render at position stated in arguments
        screen.blit(text_surface, (cls.input_rect.x +
                                   5, cls.input_rect.y + 5))
        # set width of textfield so that text cannot get
        # outside of user's text input
        cls.input_rect.w = max(100, text_surface.get_width() + 50)
    ...
    #                 print("Color failure!")
        ...
        # print(str(count))
    @classmethod
    def drawSprites(cls, sf, spriteArray, tileArray):
