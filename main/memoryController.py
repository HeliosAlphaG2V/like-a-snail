...
from _register.lib import registerFlags, registerC, getAF, getBC, getHL, getDE, setAF, setBC, setDE, setHL #@UnresolvedImport
...
class MemCntr:
    ...
    def push(self, value):     
        ...
        logger.info("PUSH (PC " + format(self.getPC(), '04X') + "): " + format(self.getSP(), '04X') + " = " + format(value, '04X'))
        ...
        if( address < 32768 ) & ( self.biosActive == False ):
            ...
            if( address == 65360 ) & ( value == 1 ): # 0xFF50 = 65.360
                ...
                for i in range(0, 160): # 0 -> 0xA0
                    self.memory[65024 + i] = self.memory[startAddress + i] # 0xFE00
            ...
            # Check if interrupt is requested
                ...
                if( (self.memory[0xFFFF] > 0) & (self.bInterruptOn) & (self.memory[0xFF0F] != 0xE0) ):
                    ...
                    logger.info("Interrupt request: " + format(self.getPC(), '04X') + " --> " + format(self.oldPC, '04X') + ", type: " + format(value, '08b') )
                ...
                logger.info("Interrupt flag is set to: " + format(self.memory[0xFF0F], '08b'))
                return
            
            if(value > 0xFF):
                print("Can not set memory value because it is to high: " + str(value) + " Register A: " + str(self.getR8(R8ID.A)))
                print(format(self.getPC() - 1, '04X') + " PC -1 opCode: " + format(self.memory[self.getPC() - 1], '04X'))
                print(format(self.getPC() - 2, '04X') + " PC -2 opCode: " + format(self.memory[self.getPC() - 2], '04X'))
            else:
...
#         self.incPC()
    ...
    def loadBooatloader(self):
        ...
        self.bBootloader.fromfile( fBootloader,
                                   os.path.getsize(strBootloaderPath));
